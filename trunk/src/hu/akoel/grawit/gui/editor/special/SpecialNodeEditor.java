...
public class SpecialNodeEditor extends DataEditor{
	
	private static final long serialVersionUID = 165396704460481021L;
	
	private Tree tree;
	private SpecialNodeDataModel nodeForModify;
	...
	public SpecialNodeEditor( Tree tree, SpecialNodeDataModel selectedNode ){

		super( SpecialNodeDataModel.getModelNameToShowStatic() );
	...
	public void save() {
		...
		if( fieldName.getText().length() == 0 ){
			...
			for( int i = 0; i < childrenCount; i++ ){
				...
				//Ha Node-rol van szo
				if( levelNode instanceof SpecialNodeDataModel ){
					
					//Ha azonos a nev
					if( ((SpecialNodeDataModel) levelNode).getName().equals( fieldName.getText() ) ){
		...
		//Ha nem volt hiba akkor a valtozok veglegesitese
			...
			if( null == mode ){
				...
				SpecialNodeDataModel newPageBaseNode = new SpecialNodeDataModel( fieldName.getText(), fieldDetails.getText() );				
